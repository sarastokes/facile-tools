...
if ~exist('lambdaMax','var')
    lambdaMax = [563.9 536.27 432.47 493.30]; % Default values for macaque (LMSR).
    % Baylor et al 1987 fit: 561 531 430
end
if ~exist('OD','var')
    if length(lambdaMax) == 4
        OD = [0.17*ones(1,3) 0.35];
    else
        OD = 0.17*ones(size(lambdaMax));
    end
end

receptorSpectra = zeros(length(lambdaMax), length(wavelength)); 
for a = 1 : length(lambdaMax)
    receptorSpectra(a,:) = spectsens(lambdaMax(a), OD(a), 'alog', wavelength(1), wavelength(end), length(wavelength)-1);
end
