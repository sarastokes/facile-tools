...
%
%plot_graph(names,from,to,'-edgecomments',c)
%addes cellstring array comments to each edge (line between nodes)
%
...
%
%plot_graph(names,from,to,'-shape','rsrsrrrsr')
%displays nodes with either (r)ounded or (s)harp corners
%
%plot_graph(names,from,to,'-fontsize',12)
%uses specified fontsize
%default: scales font to fit node boxes
%
%outputs:
%opt = plot_graph...
...
while i<numel(varargin)
    ...
    switch val
        ...
        case {'-comments','-comment'}; opt.nodecomments = varargin{i+1}; i=i+1;
        case {'-edgecomment','-edgecomments'}; opt.edgecomments = varargin{i+1}; i=i+1;
        ...
        otherwise
            ...
            if iscellstr(val); opt.nodecomments = val; continue; end
            if ishghandle(val); opt.fig.axes = val; continue; end
            disp(val); error('unknown option');
    end
end
%20161008 force all options to be 1xn vectors
%has already been done above for for:
%names,namew,from,to,node_col,edge_col,node_round
opt.nodecomments = opt.nodecomments(:)';
opt.edgecomments = opt.edgecomments(:)';
...
if ~isequal(size(opt.names),size(opt.nodecomments)) && numel(opt.nodecomments)
    error('comments are wrong size');
end
if ~isequal(size(opt.deps.from),size(opt.edgecomments)) && numel(opt.edgecomments);
    error('edgecomments are wrong size');
end
...
% sort edges to be in same order as original from/to input
% for each edge, find the endpoint nodes it points to
% then find a from/to with the same endpoint nodes
% these may not be unique (i.e. there may be more than one line
% between the same two nodes), so need a "used" filter too
from = opt.deps.from;
to   = opt.deps.to;
for i=1:numel(edges)
    %from edge, find names of two nodes at either end
    %use these names to look up which index this is into original input
    [tf,loc] = ismember({edges(i).tail edges(i).tip},opt.namemw);
    if ~all(tf); keyboard; end
    %loc is 1x2 of from/to endpoint as index into original names
    %use this to find from/to pair that match this edge
    %there may be more than one match, use the first
    tf = from==loc(1) & to==loc(2);
    if ~any(tf); keyboard; end
    I(i) = find(tf,1,'first');
    from(I(i)) = nan; %remove them, so we won't use this pair again
    to(I(i)) = nan;
end
if ~isequal(sort(I),1:numel(I)); keyboard; end
[~,order] = sort(I);
edges = edges(order);
%now read values from edges structure
nodeindex = [opt.node(:).index];
for i=1:numel(edges)
    %index into original from/to pair
    %this is simply i because we sorted it above
    opt.edge(i).edgeindex = i;
    %index of the two endpoints into original names
    [tf,loc] = ismember({edges(i).tail edges(i).tip},opt.namemw);
    if ~all(tf); keyboard; end
    opt.edge(i).nameindex = loc;
    %names of these endpoints
    opt.edge(i).endpoints = opt.names(opt.edge(i).nameindex);
    %index of the two endpoints into nodes on figure
    opt.edge(i).nodeindex(1) = find(nodeindex==opt.edge(i).nameindex(1));
    opt.edge(i).nodeindex(2) = find(nodeindex==opt.edge(i).nameindex(2));
    %x/y points for this line
    xy = str2num(edges(i).coords);
    x = xy(1:2:end)*opt.fig.shape*opt.fig.scale;
    y = xy(2:2:end)*opt.fig.scale;
    %fix line direction
    %all lines are left-to-right by default
    %check position of original lines to see if this needs to be reverse
    xs = [opt.node(opt.edge(i).nodeindex).x];
    ys = [opt.node(opt.edge(i).nodeindex).y];
    if xs(2)<xs(1);
        x = x(end:-1:1);
        y = y(end:-1:1);
    end
    %unobserved edge case, if both are same x
    %if you get this, please send me an email so I can see how to deal with
    %it
    if xs(2)==xs(1) && ys(2)~=ys(1); keyboard; end
    
    %this ignores edges that point back to the same node
...
val = [opt.edge.nameindex];
name_from = val(1:2:end);
name_to = val(2:2:end);
...
if numel(loc)
    ...
    %highlight all links to as blue
    %blue
    for i = find(ismember(name_to,loc))
        set(opt.edge(i).handle,'Color',[0 .5 1]);
    end
    %highlight all links from as red
    %red
    for i = find(ismember(name_from,loc))
...
function opt = sub_node_comment_add(opt)
if isempty(opt.nodecomments); return; end
for i = 1:numel(opt.node)
    h = opt.node(i).handle_text;
    set(h,'String',{get(h,'String'),opt.nodecomments{opt.node(i).index}})
end
end
function opt = sub_edge_comment_create(opt)
if isempty(opt.edgecomments); return; end
for i=1:numel(opt.edge)
    x = opt.edge(i).handle.XData;
    y = opt.edge(i).handle.YData;
    I = floor(numel(x)/2);
    x = x(I);
    y = y(I);
    h = text(x,y,opt.edgecomments{i});
    set(h,'Color',get(opt.edge(i).handle,'Color'));
    set(h,'FontSize',get(opt.node(1).handle_text,'FontSize')/2);
    set(h,'HorizontalAlignment','center')
    opt.edge(i).handle_text = h;
end
end
...
%done 20150527 comment for individual links
%done 20161008 resizes all inputs to 1xn vectors
%done 20161008 add commenting on edges
%done 20161008 bugfix node comments were not in correct order, thanks Zi-Xiang WANG
...
%todo scale arrows to fit figure better
%todo use valign on edge text so it appears above/below line, but still
%attached to it
%current version 20161008