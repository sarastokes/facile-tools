function varargout = printStat(vec, useSD, showN, addMedian)
% PRINTSTAT
%
% Description:
%   Print basic stats to cmd line in format: MEAN +- SD (N)
%
% Input:
%	vec 		Data (vector, or matrix with rows = separate data)
% Optional input:
%	useSD		Use SD instead of SEM (default = true)
%   showN       Include the number of samples (default = true)
%   addMedian   Include the median value (default = false)
%
% Output:
...
%   21May2024 - SSP - added median display option
% --------------------------------------------------------------------------
    ...
    for i = 1:size(vec, 2)
        ...
        if addMedian
            str = [str, sprintf(', median = %.3f', median(vec(:,i)))];
        end
