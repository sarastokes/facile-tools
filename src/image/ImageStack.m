...
% 29Sept2017 - SSP
    ...
    methods
        function obj = ImageStack(imPath, nodes, filtFcn)
            if nargin < 2
                nodes = [];
            end
            if nargin < 3
                filtFcn = @(x) endsWith(x, [".png", ".tif", ".tiff", ".jpg"]);
            end
                ...
                if isstring(nodes)
                    ...
                    for i = 1:size(nodes,1)
                        imnodes = cat(1, imnodes,...
                            ImageNode(fullfile(imPath, nodes(i,:))));
                    end
        ...
        function v = video(obj, fName, varargin)
            ...
            if ~contains(fName, filesep)
                fName = fullfile(fileparts(obj.head.filePath), fName);
            end
